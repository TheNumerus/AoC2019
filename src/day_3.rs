...
pub fn solve() {
    let mut wire_1 = Vec::new();
    for step in WIRE_1.split(",") {
        wire_1.push(Step::parse_step(step));
    }

    let mut wire_2 = Vec::new();
    for step in WIRE_2.split(",") {
        wire_2.push(Step::parse_step(step));
    }

    let points_1 = Point::generate_points(&wire_1);
    let points_2 = Point::generate_points(&wire_2);

    let mut intersections = Vec::new();

    for (step_num_1, point_1) in points_1.iter().enumerate() {
        for (step_num_2, point_2) in points_2.iter().enumerate() {
            if point_1 == point_2 {
                intersections.push((point_1, step_num_1, step_num_2));
            }
        }
    }
...
impl Point {
    fn manhattan_dist(one: &Point, two: &Point) -> u32 {
        ((one.0 - two.0).abs() + (one.1 - two.1).abs()) as u32
    }

    fn generate_points(steps: &[Step]) -> Vec<Point> {
        use StepDir::*;
        let mut active_point = Point(0, 0);
...
impl Step {
    fn parse_step(step: &str) -> Step {
        use StepDir::*;
        let len = step[1..].parse().unwrap();

        let dir = match &step[0..1] {
            "L" => Left,
            "R" => Right,
            "U" => Up,
            "D" => Down,
            _ => panic!("invalid input on day 3")
        };
        Step{len, dir}
    }
}
